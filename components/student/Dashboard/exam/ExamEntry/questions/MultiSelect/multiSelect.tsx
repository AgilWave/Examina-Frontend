...
export function MultiSelectQuestion({
  ...
  const safePlayVideo = () => {
    ...
    if (playAttempts >= 3) {
      console.log(
        "Maximum play attempts reached, user may need to click manually"
      );
      return;
    }
    ...
    try {
      ...
      if (playPromise !== undefined) {
        playPromise
          .then(() => {
            console.log(
              "Video playing successfully after",
              playAttempts + 1,
              "attempts"
            );
          })
  ...
  const startCamera = async () => {
    try {
      ...
      if (videoRef.current) {
        ...
        setCameraActive(true); // Use oncanplay event to call our safe play function
      ...
      const checkAndStartCamera = async () => {
        try {
          ...
          if (navigator.permissions && navigator.permissions.query) {
            ...
            console.log(
              "Permissions API not available, trying direct camera access"
            );
            startCamera();
          }
        } catch (error) {
