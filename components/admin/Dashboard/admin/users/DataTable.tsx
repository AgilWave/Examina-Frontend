...
import { getAllAdminUsers } from "@/services/User/getAllAdminUsers";
...
export function DataTable() {
  ...
  const fetchData = async (page: number) => {
    ...
    try {
      ...
      if (response.isSuccessful) {
        ...
        dispatch(setUserPage(response.paginationInfo.page));
        dispatch(setUserTotalPages(response.paginationInfo.totalPages));
        dispatch(setUserNextPage(response.paginationInfo.nextPage));
        dispatch(setUserPrevPage(response.paginationInfo.page - 1));
      } else {
        setData([]);
        toast.error(response.message);
  ...
  useEffect(() => {
    dispatch(
      setUserPage(JSON.parse(sessionStorage.getItem("userPage") || "1"))
    );
    dispatch(
      setUserTotalPages(
        JSON.parse(sessionStorage.getItem("totalUserPages") || "0")
      )
    );
    dispatch(
      setUserNextPage(
        JSON.parse(sessionStorage.getItem("nextUserPage") || "-1")
      )
    );
    dispatch(
      setUserPrevPage(
        JSON.parse(sessionStorage.getItem("prevUserPage") || "-1")
      )
    );
    dispatch(
