...
import ViewUserDialog from "./view-student";
...
export function DataTable() {
  ...
  const [isBlacklisted] = useQueryState("isBlacklisted");
  const [batchName] = useQueryState("batchName");
  ...
  const fetchData = async (page: number) => {
    ...
    try {
      ...
      if (response.isSuccessful) {
        ...
        dispatch(setStudentPage(response.paginationInfo.page));
        dispatch(setStudentTotalPages(response.paginationInfo.totalPages));
        dispatch(setStudentNextPage(response.paginationInfo.nextPage));
        dispatch(setStudentPrevPage(response.paginationInfo.page - 1));
      } else {
        setData([]);
        toast.error(response.message);
  ...
  useEffect(() => {
    const page = JSON.parse(sessionStorage.getItem("studentPage") || "1");
    fetchData(page);
  }, [filterApplied]);
  ...
  useEffect(() => {
    dispatch(
      setStudentPage(JSON.parse(sessionStorage.getItem("studentPage") || "1"))
    );
    dispatch(
      setStudentTotalPages(
        JSON.parse(sessionStorage.getItem("totalStudentPages") || "0")
      )
    );
    dispatch(
      setStudentNextPage(
        JSON.parse(sessionStorage.getItem("nextStudentPage") || "-1")
      )
    );
    dispatch(
      setStudentPrevPage(
        JSON.parse(sessionStorage.getItem("prevStudentPage") || "-1")
      )
    );
    dispatch(
  ...
  const handleNext = async () => {
    if (page.students.nextPage > page.students.page) {
      const curPage = page.students.nextPage;
      fetchData(curPage);
    }
  };

  const handlePrev = async () => {
    if (page.students.prevPage >= 0) {
      const curPage = page.students.prevPage;
      fetchData(curPage);
    }
  };
      ...
      <div className="flex flex-col gap-4 w-full">
        ...
        <div className="flex items-center justify-center gap-4 mt-4">
          <Button
            ...
            size="sm"
            onClick={handlePrev}
            disabled={page.students.prevPage <= 0}
          >
            Previous
          </Button>
          <span className="text-sm font-medium text-gray-700 dark:text-gray-200">
            Page {page.students.page} of {page.students.totalPages}
          </span>
          <Button
            variant="outline"
            size="sm"
            onClick={handleNext}
            disabled={
              page.students.nextPage === -1 || page.students.nextPage === null
            }
          >
            Next
          </Button>
