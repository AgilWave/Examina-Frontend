...
export function DataTable() {
  ...
  const fetchData = async (page: number) => {
    ...
    try {
      ...
      if (response.isSuccessful) {
        ...
        dispatch(setLecturePage(response.paginationInfo.page));
        dispatch(setLectureTotalPages(response.paginationInfo.totalPages));
        dispatch(setLectureNextPage(response.paginationInfo.nextPage));
        dispatch(setLecturePrevPage(response.paginationInfo.page - 1));
      } else {
        setData([]);
        toast.error(response.message);
  ...
  useEffect(() => {
    const page = JSON.parse(sessionStorage.getItem("lecturePage") || "1");
    fetchData(page);
  }, []);
  ...
  useEffect(() => {
    dispatch(
      setLecturePage(JSON.parse(sessionStorage.getItem("lecturePage") || "1"))
    );
    dispatch(
      setLectureTotalPages(
        JSON.parse(sessionStorage.getItem("totalLecturePages") || "0")
      )
    );
    dispatch(
      setLectureNextPage(
        JSON.parse(sessionStorage.getItem("nextLecturePage") || "-1")
      )
    );
    dispatch(
      setLecturePrevPage(
        JSON.parse(sessionStorage.getItem("prevLecturePage") || "-1")
      )
    );
    dispatch(
  ...
  const handleNext = async () => {
    if (page.lectures.nextPage > page.lectures.page) {
      const curPage = page.lectures.nextPage;
      fetchData(curPage);
    }
  };

  const handlePrev = async () => {
    if (page.lectures.prevPage >= 0) {
      const curPage = page.lectures.prevPage;
      fetchData(curPage);
    }
  };
      ...
      <div className="flex flex-col gap-4 w-full">
        ...
        <div className="flex items-center justify-center gap-4 mt-4">
          <Button
            ...
            size="sm"
            onClick={handlePrev}
            disabled={page.lectures.prevPage <= 0}
          >
            Previous
          </Button>
          <span className="text-sm font-medium text-gray-700 dark:text-gray-200">
            Page {page.lectures.page} of {page.lectures.totalPages}
          </span>
          <Button
            variant="outline"
            size="sm"
            onClick={handleNext}
            disabled={
              page.lectures.nextPage === -1 || page.lectures.nextPage === null
            }
          >
            Next
          </Button>
