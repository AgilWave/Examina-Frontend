...
import { FacultyName } from "./filter/selectors/FacultyName";
import { getAllModules } from "@/services/questionBank/getAllModules";
import { CourseName } from "./filter/selectors/CourseName";
...
export function DataTable() {
  ...
  const [Faculty] = useQueryState("faculty", parseAsString);
  const [Course] = useQueryState("course", parseAsString);
  ...
  useEffect(() => {
    const page = JSON.parse(sessionStorage.getItem("coursePage") || "1");
  ...
  useEffect(() => {
    dispatch(
      setQuestionBankPage(
        JSON.parse(sessionStorage.getItem("coursePage") || "1")
      )
    );
    dispatch(
      setQuestionBankTotalPages(
                ...
                rows.map((row) => {
                  return (
                    <TableRow
                      key={row.id}
                      onClick={() => {
                        router.push(`/admin/dashboard/exams/questions-bank/view-questions?id=${row.original.id}&module=${row.original.name}`);
                      }}
                    >
