...
const CreationPage: React.FC<CreationPageProps> = ({ onQuestionAdded }) => {
  ...
  const handleQuestionFieldChange = (
    ...
    if (newQuestion) {
      if (field === "correctAnswers") {
        ...
        setNewQuestion({
          ...newQuestion,
          correctAnswerClarifications: clarifications,
        });
      } else {
  ...
  const handleRemoveWrongAnswer = (index: number) => {
    if (newQuestion) {
      const updatedWrongAnswers = newQuestion.wrongAnswers.filter(
        (_, i) => i !== index
      );
      const updatedWrongClarifications =
        newQuestion.wrongAnswerClarifications.filter((_, i) => i !== index);
      setNewQuestion({
        ...newQuestion,
  ...
  const handleRemoveCorrectAnswer = (index: number) => {
    if (newQuestion) {
      ...
      const updatedCorrectClarifications =
        newQuestion.correctAnswerClarifications.filter((_, i) => i !== index);
      setNewQuestion({
        ...newQuestion,
  ...
  const handleWrongClarificationChange = (index: number, value: string) => {
    if (newQuestion) {
      const updatedClarifications = [
        ...(newQuestion.wrongAnswerClarifications || []),
      ];
  ...
  const validateQuestion = (question: QuestionData): boolean => {
    ...
    if (
      ...
      const validWrongAnswers = question.wrongAnswers.filter((answer) =>
        answer.trim()
      ).length;
    ...
    if (newQuestion && validateQuestion(newQuestion)) {
      ...
      if (newQuestion.category === "mcq") {
        if (newQuestion.questionType === "Multiple Choice") {
          const correctAnswers = newQuestion.correctAnswers.filter((answer) =>
            answer.trim()
          );
          ...
          answerOptions = correctAnswers.map((text, idx) => ({
            text,
            clarification:
              newQuestion.correctAnswerClarifications?.[idx] ||
          ...
          answerOptions = [
            ...
            ...newQuestion.wrongAnswers
              ...
              .map((answer, index) => ({
                text: answer,
                clarification:
                  newQuestion.wrongAnswerClarifications?.[index] || "",
                isCorrect: false,
              })),
          ];
        } else {
      ...
      setQuestions([
        ...questions,
        { ...newQuestion, saved: true, collapsed: true },
      ]);
  ...
  }; // Function to add all created questions to exam
  const addQuestionsToExam = () => {
    if (questions.length > 0) {
      setAddToExamDialogOpen(true);
    } else {
      toast.error("No questions available to add to exam");
    }
  };

  const confirmAddQuestionsToExam = () => {
    // Here you would usually dispatch an action to add these questions to the exam
    // For now, we'll just show a success message
    toast.success(`${questions.length} questions added to exam successfully`);
    setAddToExamDialogOpen(false);
  };
  ...
  return (
    <div className="flex flex-col gap-6 dark:text-gray-100 creation-page">
            ...
            {category && questionType ? (
              <ScrollArea className="h-[50vh] pr-4">
                <div className="space-y-6">
                  <Badge
                    variant="outline"
                    className="mb-4 font-normal dark:text-gray-300 dark:border-gray-600 bg-teal-50 dark:bg-teal-900/20 text-teal-700 dark:text-teal-400"
                  >
                    {category} - {questionType}
                  </Badge>
                  ...
                  <QuestionTextInput
                    ...
                    onChange={(val) =>
                      handleQuestionFieldChange("questionText", val)
                    }
                    ...
                    (questionType === "Multiple Choice" ||
                      ...
                      <div className="space-y-4">
                        <AnswerSection
                          ...
                          correctAnswerClarifications={
                            newQuestion?.correctAnswerClarifications || []
                          }
                          wrongAnswerClarifications={
                            newQuestion?.wrongAnswerClarifications || []
                          }
                          onCorrectAnswerChange={(val) =>
                            handleQuestionFieldChange("correctAnswers", val)
                          }
                          onCorrectClarificationChange={(val) =>
                            handleQuestionFieldChange(
                              "correctAnswerClarifications",
                              val
                            )
                          }
                          onWrongAnswerChange={(index, val) =>
                            handleWrongAnswerChange(index, val)
                          }
                          onWrongClarificationChange={(index, val) =>
                            handleWrongClarificationChange(index, val)
                          }
                    ...
                    {newQuestion?.attachment ? (
                      <div className="space-y-2">
                        <div className="flex items-center gap-2 p-2 border rounded-md dark:border-gray-600 bg-gray-50 dark:bg-gray-800">
                          {getFileType(newQuestion.attachmentName || "") ===
                          "image" ? (
                        ...
                        {renderAttachmentPreview(
                          newQuestion.attachmentName,
                          newQuestion.attachmentPreviewUrl
                        )}
                      </div>
                    ) : (
                      <Button
                      ...
                      {currentQuestion.category === "mcq" && (
                        <div className="space-y-4">
                          ...
                          <div>
                            ...
                            <div className="space-y-2">
                              {currentQuestion.wrongAnswers.map(
                                (answer, ansIdx) => (
                                  <Card
                                    key={ansIdx}
                      ...
                      {currentQuestion.attachment && (
                        <div>
                          ...
                          <div className="space-y-2">
                            ...
                            {renderAttachmentPreview(
                              currentQuestion.attachmentName,
                              currentQuestion.attachmentPreviewUrl
                            )}
                          </div>
                        </div>
                      )}
      ...
      <Card className="bg-card h-[58vh] overflow-auto scrollbar-custom">
        <CardHeader className="pb-3">
          ...
          <CardTitle className="flex justify-between items-center dark:text-gray-100">
            <span>Created Questions ({questions.length})</span>
          ...
          {questions.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {questions.map((q, idx) => (
                <Card
                  key={q.id}
                  className="h-full flex flex-col dark:bg-black/50 hover:shadow-lg transition-shadow duration-200"
                >
                  <div className="p-4 flex flex-col h-full">
                    <div className="flex justify-between items-center">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge
                          variant="outline"
                          className="font-normal text-xs dark:border-gray-600 dark:text-gray-300"
                        >
                    ...
                    <div className="mt-3 pt-3 border-t dark:border-gray-700">
                      <div className="flex justify-between items-center">                        <span className="text-xs text-gray-500 dark:text-gray-400">
                          {q.category === "mcq" &&
                            (q.questionType === "Multiple Choice"
                              ? `${
                                  q.correctAnswers.filter((a) => a.trim())
                                    .length +
                                  q.wrongAnswers.filter((a) => a.trim()).length
                                } total options`
            ...
            <div className="flex flex-col items-center justify-center h-[40vh] text-center">
              <FileQuestion className="h-10 w-10 text-teal-500 mb-4" />
      ...
      {/* Add to Exam Confirmation Dialog */}
      <Dialog open={addToExamDialogOpen} onOpenChange={setAddToExamDialogOpen}>
        <DialogContent className="sm:max-w-md bg-white dark:bg-black/50">
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to add {questions.length} question
              {questions.length !== 1 ? "s" : ""} to your exam?
            </DialogDescription>
          </DialogHeader>
          <div className="flex justify-end space-x-2 mt-4">
            <Button
              onClick={confirmAddQuestionsToExam}
            ...
            <Button
              variant="outline"
              onClick={() => setAddToExamDialogOpen(false)}
              className="gap-2"
            >
              Cancel
            </Button>
