...
import ViewUserDialog from "./view-faculty";
...
export function DataTable() {
  ...
  const fetchData = async (page: number) => {
    ...
    try {
      ...
      if (response.isSuccessful) {
        ...
        dispatch(setFacultyPage(response.paginationInfo.page));
        dispatch(setFacultyTotalPages(response.paginationInfo.totalPages));
        dispatch(setFacultyNextPage(response.paginationInfo.nextPage));
        dispatch(setFacultyPrevPage(response.paginationInfo.page - 1));
      } else {
        setData([]);
        toast.error(response.message);
  ...
  useEffect(() => {
    dispatch(
      setFacultyPage(JSON.parse(sessionStorage.getItem("coursePage") || "1"))
    );
    dispatch(
      setFacultyTotalPages(
