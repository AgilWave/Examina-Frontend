...
export function ConfirmDeleteDialog() {
  ...
  const handleSubmit = async (e: React.MouseEvent<HTMLButtonElement>) => {
    setLoaderOpen(true);
    e.preventDefault();
    if (userId !== faculty.viewFaculty?.id.toString()) {
    ...
    try {
      ...
      const response = await axios.delete(
        `${BACKEND_URL}/faculty/Interact/${id}`,
        { headers }
      );
          ...
          <TooltipProvider delayDuration={200}>
            <Tooltip>
              ...
              <TooltipContent
                ...
                side="bottom"
              >
                <p className="p-1">Delete faculty</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </AlertDialogTrigger>
            ...
            <AlertDialogDescription className=" text-[14px] font-[500]">
              <div className="flex flex-col justify-center items-center w-full  py-4">
                <p className="w-full text-center mb-3 2xl:text-[14px] text-[12px]">
                  Please enter the Id of the faculty to confirm the action
                </p>
