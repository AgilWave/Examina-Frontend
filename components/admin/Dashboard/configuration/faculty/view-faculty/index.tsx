...
import { getFacultyByID } from "@/services/Faculty/getFacultyByID";
import CreateUser from "./dialogs/UpdateFaculty";
...
function ViewUserDialog() {
  ...
  const fetchUser = async () => {
    if (dialog.viewDialogId !== undefined) {
      try {
        await getFacultyByID(dispatch, dialog.viewDialogId);
      } catch (err: any) {
        console.log(err);
      }
    }
  ...
  const headerContent = (
    <div className="flex items-center justify-between w-full">
      <div className="w-full md:w-auto">
        Faculty Details #{faculty.viewFaculty?.id}
      </div>
  ...
  const footerContent = !faculty.editBlocked && (
    <>
      <Button
        variant="outline"
