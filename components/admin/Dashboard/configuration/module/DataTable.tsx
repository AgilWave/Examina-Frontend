...
import ViewUserDialog from "./view-module";
...
export function DataTable() {
  ...
  const fetchData = async (page: number) => {
    ...
    try {
      ...
      if (response.isSuccessful) {
        ...
        dispatch(setModulePage(response.paginationInfo.page));
        dispatch(setModuleTotalPages(response.paginationInfo.totalPages));
        dispatch(setModuleNextPage(response.paginationInfo.nextPage));
        dispatch(setModulePrevPage(response.paginationInfo.page - 1));
      } else {
        setData([]);
        toast.error(response.message);
  ...
  useEffect(() => {
    const page = JSON.parse(sessionStorage.getItem("modulePage") || "1");
    fetchData(page);
  }, [filterApplied]);
  ...
  useEffect(() => {
    dispatch(
      setModulePage(JSON.parse(sessionStorage.getItem("modulePage") || "1"))
    );
    dispatch(
      setModuleTotalPages(
        JSON.parse(sessionStorage.getItem("totalmodulePages") || "0")
      )
    );
    dispatch(
      setModuleNextPage(
        JSON.parse(sessionStorage.getItem("nextmodulePage") || "-1")
      )
    );
    dispatch(
      setModulePrevPage(
        JSON.parse(sessionStorage.getItem("prevmodulePage") || "-1")
      )
    );
    dispatch(
  ...
  const handleNext = async () => {
    if (page.module.nextPage > page.module.page) {
      const curPage = page.module.nextPage;
      fetchData(curPage);
    }
  };

  const handlePrev = async () => {
    if (page.module.prevPage >= 0) {
      const curPage = page.module.prevPage;
      fetchData(curPage);
    }
  };
