...
import { setViewCourseModule } from "@/redux/features/CourseSlice";
import {
  Table,
  TableBody,
  TableCell,
...
export function ModuleSelector() {
  ...
  useEffect(() => {
      getModules();
  }, [allModules.length,course.viewCourse.facultyId]);
  ...
  const handleAddModule = () => {
    ...
    if (!moduleIds.includes(selectedId)) {
      // Add to Redux store
      dispatch(setViewCourseModule([...moduleIds, selectedId]));
      // Reset selection
      setSelectedModuleId("");
    }
  };
  ...
  const handleDeleteModule = (moduleId: number) => {
    ...
    dispatch(setViewCourseModule(updatedModuleIds));
  };
