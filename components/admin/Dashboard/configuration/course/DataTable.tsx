...
import ViewUserDialog from "./view-course";
...
export function DataTable() {
  ...
  const fetchData = async (page: number) => {
    ...
    try {
      ...
      if (response.isSuccessful) {
        ...
        dispatch(setCoursePage(response.paginationInfo.page));
        dispatch(setCourseTotalPages(response.paginationInfo.totalPages));
        dispatch(setCourseNextPage(response.paginationInfo.nextPage));
        dispatch(setCoursePrevPage(response.paginationInfo.page - 1));
      } else {
        setData([]);
        toast.error(response.message);
  ...
  useEffect(() => {
    dispatch(
      setCoursePage(JSON.parse(sessionStorage.getItem("coursePage") || "1"))
    );
    dispatch(
      setCourseTotalPages(
